...
import 'register_parking_usecase_test.mocks.dart';

@GenerateMocks([ParkingRepository])
void main() {
  ParkingRepository mockParkingRepository = MockParkingRepository();
  RegisterParkingUseCase usecase =
      RegisterParkingUseCase(repository: mockParkingRepository);

  test('should register a new object', () async {
    when(mockParkingRepository.registerParking(testParkingEntity))
        .thenAnswer((_) => Future.value(const Right(null)));
    final result =
        await usecase(const Params(parkingEntity: testParkingEntity));
    expect(result, const Right(null));
    verify(mockParkingRepository.registerParking(testParkingEntity));
